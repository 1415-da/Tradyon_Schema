...
export default function ReviewTable() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        // Handle development server restart scenario
        if (data.details === 'Development server restart detected - in-memory data was lost') {
          throw new Error('Server restart detected. Please click "Back to Home" and start the process again.');
        }
        throw new Error(data.error || 'Failed to save and submit');
      }

      // Navigate to success page with all export information
      const params = new URLSearchParams({
        ...
        productCount: (data.productCount || 0).toString()
      });
        ...
        <div className="mb-6 bg-red-50 border border-red-200 rounded-xl p-4">
          <div className="flex items-start">
            ...
            <div className="flex-1">
              ...
              {error.includes('Server restart detected') && (
                <button
                  onClick={() => {
                    resetStore();
                    router.push('/');
                  }}
                  className="mt-3 inline-flex items-center px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors"
                >
                  <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12a9 9 0 0115-6.928L21 3m0 0v6m0-6h-6m6 0L15 9" />
                  </svg>
                  Back to Home
                </button>
              )}
