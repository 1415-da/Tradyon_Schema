...
export async function POST(request: NextRequest) {
  try {
    const { companyName, productName, categoryName } = await request.json();
    ...
    if (!company) {
      console.log('API add-category: Company not found, recreating...');
    ...
    if (!company.products[normalizedProductName]) {
      console.log('API add-category: Product not found, recreating...');
    ...
    if (product.categories[normalizedCategoryName]) {
      return NextResponse.json(
        { error: 'Category already exists for this product' },
        { status: 409 }
      );
    }

      // For predefined categories, get their default options based on product
