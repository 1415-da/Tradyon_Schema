...
export async function POST(request: NextRequest) {
  try {
    const { companyName, productName, categoryName, option } = await request.json();
    ...
    if (!company) {
      console.log('API add-option: Company not found, recreating...');
    ...
    // If product doesn't exist (server restart), recreate it
    if (!company.products[normalizedProductName]) {
      console.log('API add-option: Product not found, recreating...');
    ...
    if (!category) {
      console.log('API add-option: Category not found, creating with default options...');
    ...
    if (category.options.includes(normalizedOption)) {
      return NextResponse.json(
        { error: 'Option already exists for this category' },
        { status: 409 }
      );
    }
        ...
        [normalizedProductName]: {
          categories: {
            ...
            [normalizedCategoryName]: {
              options: [...category.options, normalizedOption],
