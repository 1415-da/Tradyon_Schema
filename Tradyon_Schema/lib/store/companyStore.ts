...
interface CompanyStore {
  ...
  // Derived state for stable references
  productList: string[];
  categoryList: { [productName: string]: string[] };
  optionList: { [productCategory: string]: string[] };

  // Predefined products (available to all companies)
  predefinedProducts: string[];

  // Product sub-varieties (available to all companies)
  productSubVarieties: {
    [productName: string]: string[];
  };

  // Product-specific predefined categories (available to all companies)
  ...
  // Actions
  setCompanyName: (companyName: string, contactPerson?: string) => void;
  addProduct: (productName: string) => void;
  addCategory: (productName: string, categoryName: string) => void;
  addOption: (productName: string, categoryName: string, option: string) => void;
  setSelectedValue: (productName: string, categoryName: string, value: string) => void;
  removeProduct: (productName: string) => void;
  removeCategory: (productName: string, categoryName: string) => void;
  resetStore: () => void;
}

// Product sub-varieties
const initialProductSubVarieties = {
  'Cloves': [
    'Clove Buds',
    'Clove Flowers',
    'Clove Dust',
...
const initialPredefinedProducts = [
  'Cloves',
  'Pepper'
];

// Product-specific predefined categories (works for both major categories and sub-varieties)
const initialPredefinedCategories = {
  ...
  'Clove Buds': {
    'physical_form': [
      'Whole', 'Buds', 'Flowers', 'Crushed', 'Dried'
    ],
    'origin_country': [
      'Indonesia', 'Comoros', 'Madagascar', 'Sri Lanka'
    ],
    'processing_method': [
  ...
  'Clove Stems': {
    'origin_country': [
      'Madagascar', 'Indonesia', 'Comoros', 'Sri Lanka', 'Zanzibar'
    ],
    'use': [
  ...
  'Whole Cloves': {
    'origin_country': [
      'Indonesia', 'Madagascar', 'Comoros', 'Sri Lanka', 'Brazil', 'India', 'Zanzibar', 'Tanzania'
    ],
    'physical_form': [
      'Whole', 'Long', 'Dried', 'Buds', 'Crushed', 'Uncrushed',
      'Unmilled', 'Rolls'
    ],
    'quality_grade': [
    ...
    'processing_method': [
      ...
      'Cleaned', 'Steam', 'Eto Sterilized', 'Well Dried', 'Treated', 'Selected',
  ...
  'Bell Pepper': {
    ...
    'packaging_type': ['Carton', 'Bag', 'Tin', 'Can', 'Box']
  },
  'Black Pepper': {
    ...
    'mesh_min': ['20.0', '5.0', '28.0', '3.0', '18.0', '1.0', '12.0', '16.0', '500.0', '1.5', '150.0', '14.0', '35.0', '4.25', '34.0', '30.0', '40.0', '50.0', '60.0', '80.0', '15.0', '10.0', '1.2', '300.0', '2.5', '25.0', '1200.0', '550.0', '0.25', '2.0', '22.0', '24.0', '32.0', '0.4', '6.0', '8.0', '100.0', '62.0', '4.0', '45.0', '0.8', '570.0', '29.0', '31.0', '400.0', '8085.0', '0.5', '6.5', '7.0', '710.0', '0.6', '5001700.0', '850.0', '1000.0', '420.0', '600.0', '200.0', '48.0'],
    'mesh_max': ['20.0', '5.0', '28.0', '4.0', '18.0', '2.0', '16.0', '1700.0', '1.5', '500.0', '14.0', '35.0', '4.75', '34.0', '40.0', '30.0', '50.0', '60.0', '80.0', '15.0', '1.0', '300.0', '2.5', '25.0', '1200.0', '12.0', '550.0', '1.19', '22.0', '24.0', '32.0', '0.8', '8.0', '125.0', '100.0', '62.0', '6.0', '45.0', '10.0', '570.0', '3.0', '29.0', '31.0', '400.0', '8085.0', '49.0', '7.0', '2000.0', '12.5', '1.2', '5001700.0', '600.0', '1400.0', '850.0', '2400.0', '1180.0', '2.4', '1000.0', '2360.0', '1.25', '48.0', '85.0'],
  ...
  'Pimento': {
    'variety': ['Mexican Pimento', 'Pimento, Allspice', 'Allspice', 'Pimento', 'Pimienta Dioica', 'Guajillo', 'Pimiento Allspice', 'Piment', 'Pimienta Mexicana', 'Pimienta Nat', 'Pimiento Paprika'],
...
// Helper function to ensure categoryList has entries for all products
const ensureCategoryListSync = (productList: string[], categoryList: { [productName: string]: string[] }) => {
  const syncedCategoryList = { ...categoryList };
  productList.forEach(product => {
    if (!syncedCategoryList[product]) {
      syncedCategoryList[product] = [];
    }
  });
  return syncedCategoryList;
};
...
export const useCompanyStore = create<CompanyStore>((set, get) => ({
  ...
  categoryList: {},
  optionList: {},
  predefinedProducts: initialPredefinedProducts,
  productSubVarieties: initialProductSubVarieties,
  predefinedCategories: initialPredefinedCategories,

  setCompanyName: (companyName: string, contactPerson?: string) => {
    set({
      companyData: {
        companyName,
  ...
  addProduct: (productName: string) => {
    const { companyData, productList, categoryList } = get();
    if (!companyData) return;

    if (companyData.products[productName]) return; // Product already exists

    const newProductList = [...productList, productName];
  ...
  addCategory: (productName: string, categoryName: string) => {
    const { companyData, categoryList } = get();
    if (!companyData || !companyData.products[productName]) return;

    const product = companyData.products[productName];
    const categoryOptions = get().predefinedCategories[productName]?.[categoryName] || [];
    const currentCategories = categoryList[productName] || [];
    ...
    set({
      ...
      categoryList: {
        ...categoryList,
        [productName]: newCategories
      }
    });
  },

  addOption: (productName: string, categoryName: string, option: string) => {
    const { companyData, optionList } = get();
    if (!companyData || !companyData.products[productName]) return;
  ...
  setSelectedValue: (productName: string, categoryName: string, value: string) => {
    console.log('setSelectedValue called:', { productName, categoryName, value });
    const { companyData } = get();
    if (!companyData || !companyData.products[productName]) {
      console.log('setSelectedValue: company or product not found');
      return;
    }
    ...
    const category = product.categories[categoryName];
    if (!category) {
      console.log('setSelectedValue: category not found');
      return;
    }
    ...
    console.log('setSelectedValue: toggling', value, 'from', currentSelectedValues, 'to', newSelectedValues);
    ...
    console.log('setSelectedValue: store updated');
  },

  removeProduct: (productName: string) => {
    const { companyData, productList, categoryList, optionList } = get();
    if (!companyData) return;

    const { [productName]: removed, ...remainingProducts } = companyData.products;
    const newProductList = productList.filter(p => p !== productName);
    const { [productName]: removedCategories, ...remainingCategoryList } = categoryList;

    // Remove all option keys that start with this product name
    const newOptionList = Object.fromEntries(
      Object.entries(optionList).filter(([key]) => !key.startsWith(`${productName}-`))
    );
    ...
    set({
      ...
      productList: newProductList,
      categoryList: remainingCategoryList,
      optionList: newOptionList
    });
  },

  removeCategory: (productName: string, categoryName: string) => {
    const { companyData, categoryList, optionList } = get();
    if (!companyData || !companyData.products[productName]) return;

    const product = companyData.products[productName];
    const { [categoryName]: removed, ...remainingCategories } = product.categories;
    const currentCategories = categoryList[productName] || [];
    const newCategories = currentCategories.filter(c => c !== categoryName);

    // Remove the option key for this category
    const optionKey = `${productName}-${categoryName}`;
    const { [optionKey]: removedOptions, ...remainingOptionList } = optionList;
    ...
    set({
      ...
      optionList: remainingOptionList
    });
  },
